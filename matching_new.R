...
lmnaive_el <- lm(birth_y1 ~ married + ch1+ ch2 + ch3 + ch4 + ch5 + chsup + age_w  + Reg + nat_country, data = data_[which(data_$eligible==1),]) #
summary(lmnaive_el)
out.tex = xtable(lmnaive_el)
print(out.tex, type='latex', file=paste(outputs_matching, '/lm_naive_el.tex', sep=''), compress = FALSE) 
rm(out.tex)

lmnaive_inel <- lm(birth_y1 ~ married + ch1+ ch2 + ch3 + ch4 + ch5 + chsup + age_w + Reg + nat_country, data = data_[which(data_$eligible==0),]) #
